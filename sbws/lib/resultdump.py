...
class Result:
    ...
    def __init__(self, relay, circ, scanner_nick, t=None):
        """
...
class ResultSuccess(Result):
    def __init__(self, downloads, *a, **kw):
        super().__init__(*a, **kw)
        # self._rtts = rtts
        self._downloads = downloads
        # self._relay_average_bandwidth = relay_average_bandwidth
    ...
    # @property
    # def relay_average_bandwidth(self):
    #     return self._relay_average_bandwidth

    @staticmethod
    def from_dict(d):
        assert isinstance(d, dict)
        return ResultSuccess(d['downloads'],
            Result.Relay(
                d['fingerprint'], d['nickname'], d['address'],
    ...
    def new_dict(d):
        assert isinstance(d, dict)
        return ResultSuccess(d['bw'],
            Result.Relay(
                d['node_id'], d['nick'], d['address'],
                [], [],
                ),
            [], [],
            [])
...
class ResultDump:
    ...
    def handle_result(self, result):
        ...
        if result.type == "success":
            msg = "Success measuring {} ({}) via circuit {} and ".format(
                    result.fingerprint, result.nickname, result.circ,
                    )
        else:
            msg = "Error measuring {} ({}) via circuit {} and : {}".format(
                    result.fingerprint, result.nickname, result.circ,
                    result.msg)
